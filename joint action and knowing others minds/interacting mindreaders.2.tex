...
we shall briefly review some potential benefits of being able to identify goals.
...
identify the goals of those actions (\vpageref*{goal_ascription_verify}).
In addition,
...
what evidence grounds ascriptions of goals to actions?
...
The falsity of this claim seems to be assumed in developmental research on goal ascription.%
\footnote{ 
...
while not knowing anything about the agents' mental states.
Further,
...
Even if she had no ability to ascribe mental states, it seems she might nevertheless be in a position to acquire knowledge of the goal of the other's action.
...
\end{quote}
%
Taking this as a rough starting point,
we propose that
goal ascription can involve 
identifying which potentially desirable or beneficial outcomes 
an observed action is the best available means to achieving.
More carefully, our proposal is that:
%
\begin{idescription}
\item[(E$_1$)] actions of type $a$ are normally a means of realising outcome $G$ in situations with the salient features of this situation;
\end{idescription}
%
and:
%
\begin{idescription}
\item[(E$_{2}$)] no alternative type of action, $a'$, is available to the agent and such that actions of this type would be normally be better\footnotemark \ means of realising outcome $G$ in situations which have the salient features of this situation;
\footnotetext{
...
%
\begin{idescription}
\item[(E$_3$)] the occurrence of outcome $G$ is potentially beneficial for the agent or typically desirable to agents of this type,
and there is no other outcome, $G'$, which would be more beneficial to the agent or typically more desirable to agents of this type
where  (E$_1$) and (E$_2$) both hold of $G'$ and $a$
\end{idescription}
%
jointly constitute defeasible evidence for the conclusion that:
%
\begin{idescription}
\item[(C)] $G$ is a goal to which action $a$ is directed.
\end{idescription}
%
We further suggest that the above inference from (E$_1$)-(E$_3$) to (C) is a route to knowledge of the goals of actions in this sense:
it would be possible to know the premises without already knowing the conclusion
and 
in some cases knowing the premises might put one in a position to know the conclusion.

This proposal might be extended in various ways.

Fact that this route to knowledge exists shows that goal ascription doesn't depend on mental state ascription.

Don't suggest this is the only route to knowledge.
Not claiming to be offering a comprehensive theory of the evidential basis of goal ascription.
