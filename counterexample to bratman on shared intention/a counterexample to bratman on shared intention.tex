...
To show that  Bratman's conditions, (1)--(3) \vpageref{quote:bratman_account}, are not in fact sufficient for shared intention we need 
...
Consider the contents of the intentions concerning our J-ing in conditions (1)--(3) \vpageref{quote:bratman_account}. 
...
Rather the above  conditions, (1)--(3) \vpageref{quote:bratman_account}, must be sufficient for shared intention even for some values of J which are `neutral with respect to shared intentionality.'%
\footnote{
...
%(This is not due to ignorance: it's just how they approach their tasks.)
...
%%having agents each individually make a plan for all the agents' actions
...
%*people are typically very bad at predicting their own actions.
...
%Echoing Moran, we might put the idea roughly by saying that she is constantly estranged from the actions making up one or another area of her life.
...
%When shared agency is not involved, it is probably right to take the perspective of an outsider on others' actions when not adopting their perspective.
%When not involved in others' actions it is perhaps right to feel estranged from them.
...
%simultaneously not taking the perspective of an outsider on co-actors' actions.
...
%Beatrice and Baldric thereby fail to make use of the fact that they each intend the same thing, namely that they walk to the corner.
By contrast,
...
\footnote{
The phrase `make a plan for all the agents' actions' is used the special sense explained \vpageref{df:plan_all_actions}.
}
